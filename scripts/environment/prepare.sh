...
if [[ "$(cargo-nextest --version)" != "cargo-nextest 0.9.72" ]] ; then
  cargo install cargo-nextest --version 0.9.72 --force --locked
fi
if ! cargo deny --version >& /dev/null ; then
  cargo install cargo-deny --version 0.13.9 --force --locked
fi
...
if ! wasm-pack --help >& /dev/null ; then
  cargo install wasm-pack --version 0.10.3 --force --locked
fi
